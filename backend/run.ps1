...
$sshProcess = Start-Process -FilePath "ssh" -ArgumentList "-L", "5433:127.0.0.1:5432", "debian@51.178.30.35", "-N" -PassThru -WindowStyle Hidden -RedirectStandardInput NUL
Start-Sleep -Seconds 2
if ($sshProcess.HasExited) {
    Write-Host "ERREUR: Impossible d'etablir le tunnel SSH!" -ForegroundColor Red
    Write-Host "Verifiez votre connexion et vos cles SSH." -ForegroundColor Yellow
} else {
    Write-Host "Tunnel SSH etabli (PID: $($sshProcess.Id))" -ForegroundColor Green
}
Write-Host ""
